...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_postings_list(self, term):
        ...
        postings = self.index_file.read(term_pos[2])
        tf_list = self.index_file.read(term_pos[3])
        post_value = self.postings_encoding.decode(postings)
        tf_value = self.postings_encoding.decode_tf(tf_list)
        ret_value = [(post_value[i], tf_value[i]) for i in range(len(tf_value))]
        self.index_file.seek(0)
        return ret_value

    def get_avg_doc_length(self):
        temp_total = 0
        for i in self.doc_length.keys():
            temp_total += self.doc_length[i]

        self.total_doc_length = temp_total
        self.avg_doc_length = temp_total / len(self.doc_length)
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        """
        # TODO
        post_encode = self.postings_encoding.encode(postings_list)
        tf_encode = self.postings_encoding.encode_tf(tf_list)
        if len(self.terms) == 0:
            self.postings_dict[term] = (0, len(postings_list), len(post_encode), len(tf_encode))
        else:
            self.postings_dict[term] = (
            self.postings_dict[self.terms[-1]][2] + self.postings_dict[self.terms[-1]][0] + self.postings_dict[self.terms[-1]][3],
            len(postings_list), len(post_encode), len(tf_encode))
        self.index_file.write(post_encode)
        self.index_file.write(tf_encode)
